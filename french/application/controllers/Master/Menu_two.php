...
class Menu_two extends CI_Controller {
	...
	public function view(){
		...
		$data['title'] = "Menu";
		$data['cur_page'] = 'menu_two';
		$data['cur_sub_page'] = '';
		$data['menu'] = $this->admin->fetch_data('tbl_menu_two');

		$this->load->view('admin/inc/header', $data);
		$this->load->view('admin/pages/menu_two/view');
	...
	public function menu_add(){
		...
		$this->load->view('admin/inc/header', $data);
		$this->load->view('admin/pages/menu_two/add');
		$this->load->view('admin/inc/footer');
	}
	...
	public function saveFirstMenu(){
		...
		if( !empty($post['page_id']) && !empty($post['menu_order']) && is_numeric($post['menu_order'])) {
			...
			if( $_POST['menu_parent'] == '') {
	    		...
	    		redirect(base_url('Master/menu_two/menu_add'));
			} else{
				$data = array(
					'menu_type' => 'Page',
				...
				$this->admin->insert('tbl_menu_two', $data);
	...
	public function menu_edit($id){
		...
		$this->load->view('admin/inc/header', $data);
		$this->load->view('admin/pages/menu_two/edit');
		$this->load->view('admin/inc/footer');
	}
	...
	public function updateFirstMenu(){
		...
		if(!empty($post['menu_order']) && is_numeric($post['menu_order'])){
			...
			$this->admin->update('tbl_menu_two', $data, $post['id']);

			$this->session->set_flashdata('success','Menu is updated successfully!!');
		    redirect(base_url('Master/menu_two/menu_edit/'.$post['id']));
		} else {
			$this->session->set_flashdata('error','Page, Parent, Order, Url Can not be blank and Order Should be numeric Also!!');
		    redirect(base_url('Master/menu_two/menu_edit/'.$post['id']));
		}
	...
	public function deleteMenu($id){
		$this->db->delete('tbl_menu_two', array('id' => $id));
		$this->session->set_flashdata('success','Menu Delete Successfully!!');
	    redirect(base_url('Master/menu_two/view'));
	}
