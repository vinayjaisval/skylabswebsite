<?php
$statement = $this->db->query("SELECT * FROM tbl_menu WHERE id=".$id);
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="<?=base_url('Master/menu/view');?>" class="btn btn-primary btn-sm">View All</a>
	</div>
</section>
			...
			<div class="nav-tabs-custom">
				...
				<div class="tab-content">
      				<div class="tab-pane <?php if($menu_type == 'Page') {echo 'active';} ?>" id="tab_1">
						...
						<form class="form-horizontal" action="<?=base_url('Master/menu/updateFirstMenu')?>" method="post" enctype="multipart/form-data">
							<input type="hidden" name="current_photo" value="<?php echo $menu_img; ?>">
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											<select class="form-control select2" name="menu_parent" style="width:100%;">
												...
												<?php
    $q = $this->db->query("SELECT * FROM tbl_menu ORDER BY menu_order ASC");
                                                foreach ($q->result() as $row) {
                                                    ...
                                                    if($row->page_id==0) {      
                                                        echo '<option value="'.$row->id.'" '.$sel1.'>'.$row->menu_name.'</option>';
                                                    } else {
$r = $this->db->query("SELECT * FROM tbl_page WHERE id=".$row->page_id);
                                 foreach ($r->result() as $row1) {
                                    echo '<option value="'.$row->id.'" '.$sel1.'>'.$row1->page_name.'</option>';
                                                        }
                                                    }
                                                }
                                                ?>
									...
									<div class="form-group">
										...
										<div class="col-sm-6" style="padding-top:5px">
											<input type="file" name="menu_img">(Only jpg, jpeg, gif and png are allowed)
										</div>
										<div class="col-md-2">
											<img src="<?=base_url('assets/admin/uploads/'.$menu_img);?>" alt="<?=$menu_name;?>" style="max-width:100%;">
										</div>
									</div>	
									<div class="form-group">
      				...
      				<div class="tab-pane <?php if($menu_type == 'Other') {echo 'active';} ?>" id="tab_2">
						...
						<form class="form-horizontal" action="<?=base_url('Master/menu/updateSecondMenu');?>" method="post" enctype="multipart/form-data">
                            <input type="hidden" name="id" value="<?=$id;?>">
							<input type="hidden" name="current_photo" value="<?php echo $menu_img; ?>">
							<div class="box box-info">
								<div class="box-body">
